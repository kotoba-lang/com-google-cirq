(ns google-cirq.main
  "Kotodama WASM entrypoint for the Google Cirq clean-room actor (L4) — Clojure port.
  ...
  facts (`google_cirq.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "google_cirq")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"google_cirq.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "google_cirq-compat" :tier tier :entities entities} 200])
...
(defn start! [] :google_cirq-compat/ready)
